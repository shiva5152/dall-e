...
const RenderCards = ({ data, title }) => {
  ...
  if (data?.length > 0) {
    return (
      data.map((post) => <Card key={post._id} {...post} show={false} />)
    );
  }
...
return (
        ...
        <div>
          ...
          <form className='mt-16 max-w-3xl' onSubmit={handleSubmit}>
            <div className='flex flex-col gap-5'>
              ...
              <div>
                <div >
                <label htmlFor="resolution" className="block text-sm font-medium text-gray-900">Choose Image Resolution</label>
                  <select  id="resolution" name='resolution' onChange={handleChange} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:border-gray-300 outline-none block w-full p-3">
                    <option value="1024x1024" className='p-2 mt-4'>1024x1024</option>
                    <option selected value="512x512" className='p-2'>512x512</option>
                    <option value="256x256" className='p-2'>256x256</option>
                  </select>
                </div>
              </div>
      ...
      <section className='w-0 md:w-2/12 hidden md:flex flex-col space-y-1 mx-auto overflow-y-auto  overflow-hidden h-screen scroll-m-1 px-5' >
        <h1 className='font-bold text=[#222328] text=[32px] mx-auto'>History</h1>
        {isAuthenticated ? (
                  <RenderCards
                    data={userPosts}
                  ...
                  <RenderCards
                    data={[]}
                    title="Please login for your history"
                  />
                )}
      </section>
      </main>
